    ...
    <div style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr', fontFamily: BODY }}>
      ...
      <div style={{ display: 'grid', placeItems: 'center', padding: 40, background: '#fff' }}>
        <div style={{ width: '100%', maxWidth: 380 }}>
          ...
          {/* Google */}
          <button type="button" onClick={handleGoogle} style={{ width: '100%', padding: '12px', borderRadius: 11, border: `1px solid ${C.line}`, background: '#fff', color: C.tx, fontFamily: BODY, fontWeight: 600, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            <div style={{ flex: 1, height: 1, background: C.line }}/><span style={{ fontSize: 12, color: C.tx3, fontWeight: 600 }}>o con email</span><div style={{ flex: 1, height: 1, background: C.line }}/>
          </div>

          {info && <div style={{ background: C.excBg, color: C.exc, borderRadius: 10, padding: '10px 14px', fontSize: 13, marginBottom: 16, fontWeight: 600 }}>{info}</div>}

          {forgotMode ? (
            <form onSubmit={handleForgot}>
              <p style={{ fontSize: 13.5, color: C.tx2, marginBottom: 18, lineHeight: 1.5 }}>Ingresá tu email y te enviamos un enlace para restablecer tu contraseña.</p>
              ...
              {error && <div style={{ background: C.criticoBg, color: C.critico, borderRadius: 10, padding: '10px 14px', fontSize: 13, marginBottom: 16, fontWeight: 600 }}>{error}</div>}
              <button type="submit" disabled={loading} style={{ width: '100%', padding: '13px', borderRadius: 11, border: 'none', background: C.grad, color: '#fff', fontFamily: DISP, fontWeight: 700, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? .7 : 1 }}>
                {loading ? 'Enviando…' : 'Enviar enlace'}
              </button>
              <button type="button" onClick={() => { setForgotMode(false); setError('') }} style={{ background: 'none', border: 'none', color: C.tx3, fontSize: 13, cursor: 'pointer', marginTop: 14, padding: 0 }}>
                ← Volver al login
              </button>
            </form>
          ) : (
            <form onSubmit={handleLogin}>
              ...
              <button type="button" onClick={() => { setForgotMode(true); setError('') }} style={{ background: 'none', border: 'none', color: C.tx3, fontSize: 12.5, cursor: 'pointer', padding: '4px 0 16px', display: 'block' }}>
